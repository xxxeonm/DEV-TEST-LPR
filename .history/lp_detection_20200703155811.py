...
def recogLp(filename):
    ...
    org_img=org_img[rec_box[select][1]-20:rec_box[select][3]+rec_box[select][1]+20, rec_box[select][0]-50:100+rec_box[select][0]+400]
    rslt_detect = cv2.resize(org_img, None,fx=1.8,fy=1.8,interpolation=cv2.INTER_CUBIC+cv2.INTER_LINEAR)
    cv2.imshow('rslt_detect', rslt_detect)
