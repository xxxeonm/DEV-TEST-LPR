...
def recogLp(filename):
    ...
    for i in range(len(contours)):
        ...
        # filter out by box size <<OVERFITTED
    ...
    for i in range(len(rec_box)):
        for j in range(len(rec_box)-(i+1)):
            if rec_box[j][0] > rec_box[j+1][0]:
                ...
                rec_box[j+1] = tmp
    # measure length between rectangles TO FIND PLATE
    ...
    org_img=org_img[rec_box[select][1]-10:rec_box[select][3]+rec_box[select][1]+10,rec_box[select][0]-200:rec_box[select][0]+200] 
    cv2.imshow('split', org_img)
    ...
    rslt_recog = ''
    rslt = pytesseract.image_to_string(img_recog, lang='kor_lp_gen', config='--psm 7 --oem 2')
    for c in rslt:
