...
def detectLp(filename):
    ...
    img_recog = cv2.threshold(img_recog, thresh=0.0, maxval=255.0, type=cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    rslt_recog = pytesseract.image_to_string(img_th_plateresult, lang='kor_lp_gen', config='--psm 7 --oem 2')

    return rslt_recog

filenames = os.listdir('./base')
for filename in filenames:
    print(detectLp(filename))