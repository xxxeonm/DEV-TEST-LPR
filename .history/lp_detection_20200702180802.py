...
import pytesseract
pytesseract.pytesseract.tessrac_cmd = r"C:\Program Files\Tesseract-OCR\tesseract.exe"
...
def detectLp(filename):
    ...
    resize_img = cv2.resize(img, None,fx=1.8,fy=1.8,interpolation=cv2.INTER_CUBIC+cv2.INTER_LINEAR)
    cv2.imwrite('./rslt_detect/resize_'+filename, resize_img)
...
def recogLp(img):
    ...
    img = cv2.threshold(img, thresh=0.0, maxval=255.0, type=cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    cv2.imwrite('./rslt_detect/c_d_'+filename, img)
    result = pytesseract.image_to_string(img, lang='kor_lp_gen', config='--psm 7 --oem 2')
    print(result)


filenames = os.listdir('./base')
for filename in filenames:
    rslt_detect = detectLp(filename)
    recogLp(rslt_detect)