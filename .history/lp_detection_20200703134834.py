...
def recogLp(filename):
    ...
    plate_w = math.sqrt(a**2 + b**2)
    print("first::", first_p_box[0], ", ", first_p_box[1], "last::", last_p_box[0]+last_p_box[2], ", ",last_p_box[1]+last_p_box[3])
    p1 = np.float32([
    ...
    cv2.warpPerspective(img, matrix, (int(plate_w), int(first_p_box[3])))
    cv2.imshow('process perspective', img)
    cv2.waitKey(0)
