...
class Detection():
    def detectLp(self, filename):
        ...
        plate_w = int((c[0][0]-c[len(c)-1][0])*2)
        plate = rotate_img[rec_box[select][1]-50:rec_box[select][3]+rec_box[select][1]+50, rec_box[select][0]-50:100+rec_box[select][0]+plate_w]
        cv2.imwrite('./rslt_detect/resize_'+filename, cv2.resize(plate, None,fx=1.8,fy=1.8,interpolation=cv2.INTER_CUBIC+cv2.INTER_LINEAR))
